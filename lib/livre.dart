...
class _LivreState extends State<Livre>{
  ...
  void terminer(){
    var retour = {'Titre': this.s_titre,'Auteur': this.s_auteur, 'DatePublication': this.s_datePublication, 'Editeur': this.s_editeur, 'ISBN': this.s_ISBN, "UrlImage": this.s_urlImage, "Synopsis": this.s_synopsis, "Lu": lu, "Envie": ajouterA };
  ...
  void getInfo() async {
    ...
    if (onpenLibrary.statusCode == 200) {
      ...
      setState(() {
        try {
          ...
          //this.s_synopsis = this.s_synopsis == "Inconnu" ? (jsonResponse['description'])[0] : this.s_synopsis;
        }
        catch(erreur){
          print("\nErreur lors du décodage du json\n");
        }
    ...
    if(repGoogle || repOpenLibrary){
      ...
      setState(() {
        this.s_urlImage = "https://covers.openlibrary.org/b/isbn/" + this.s_ISBN + "-M.jpg";
        pageChargee = true;
      });
    }
